...
filename="metaphlan.estimated_count.filtered.7.bacteria.txt"
...
stat.test <- mydata %>%
  ...
  #adjust_pvalue() %>%
  mutate(y.position = 2)
bxp <- ggboxplot(mydata, x = "TreatVisit", y = "shannon", fill = "Visit")
...
bxp <- ggboxplot(mydata, x = "TreatVisit", y = "observed", fill = "Visit")
bxp + stat_pvalue_manual(stat.test,   label = "p", tip.length = 0.01, step.increase = 0.1)+ theme(legend.position = "none")+geom_jitter()
dev.off()
