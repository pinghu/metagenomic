...
png(filename = paste0(filename,".fold_cor_fdrsig.png"), width = 3880, height = 8880, res=300)
...
png(filename = paste0(filename,".fold_cor_fdrsig+.png"), width = 3880, height = 4880, res=300)
p<-    ggplot(PAll[PAll$fold_or_cor>0,],aes(y = taxon, x = fold_or_cor, color = diff_Cor)) +geom_point() +
  geom_vline(xintercept = 0, color = "yellow", size=1)+
  scale_color_brewer(palette = "Dark2", name = NULL) +
  facet_grid(cols  = vars(group), scales = "free") +
  theme_bw() +
