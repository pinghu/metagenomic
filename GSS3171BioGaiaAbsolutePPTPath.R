...
##### add in the filter to remove the novariation group
##### change name to only have first 50 characters
##########################################
rm(list=ls())
...
d2 <- dim(A2);
filename <- args[1]
#filename="pathabundance.community.relab10.clean"
library(ggplot2)
library(ggpubr)
library(rstatix)
library(dplyr)
...
for (i in 1:d[1]){
  ...
  # Check for sufficient variation and group diversity
  if (length(unique(mydata$TestTime)) < 2) {
    message("Skipping ", genename, ": only one group in TestTime.")
    next
  }
  
  if (length(unique(mydata$gene_relative)) < 2) {
    message("Skipping ", genename, ": no variation in gene_relative.")
    next
  }
  
  if (length(unique(mydata$gene_absolute)) < 2) {
    message("Skipping ", genename, ": no variation in gene_absolute.")
    next
  }
  
  # Safe Kruskal tests
  KP_Treat_relative <- tryCatch({
    kruskal.test(gene_relative ~ TestTime, data = mydata)$p.value
  }, error = function(e) {
    message("Skipping ", genename, ": kruskal.test (relative) failed: ", e$message)
    return(NA)
  })
  
  KP_Treat_absolute <- tryCatch({
    kruskal.test(gene_absolute ~ TestTime, data = mydata)$p.value
  }, error = function(e) {
    message("Skipping ", genename, ": kruskal.test (absolute) failed: ", e$message)
    return(NA)
  })
  
  if (is.na(KP_Treat_relative) || is.na(KP_Treat_absolute)) {
    next
  }
  ...
  results_list <- list()
  results_list[["TaxonName"]] <-genename_org
  ...
  p2 <- ggplot(summary_relative, aes(x = TestTime, y = mean_RA, group = TestTime, color = TestTime)) + 
    ...
    geom_errorbar(aes(ymin = mean_RA - se_RA, ymax = mean_RA + se_RA), width = 0.2,
                  position = position_dodge(0.05)) +
    labs(x = "Visit", y = paste0(genename, "(%):relative")) +
    theme_classic() +
    theme(
      legend.position = "none",
      strip.background = element_blank(),
  ...
  png(filename = paste0(genename, ".combine.png"), width = 2000, height = 1000, res = 300)
